...
#        plt.close('all')
    ...
    plt.errorbar(krup_freq, krup_e, xerr=freq_err, ecolor='r', ls='', marker='s', label=r"$\varepsilon$ Krupar et al. 2020")
    plt.loglog(freq_list, e_krup_list,'o', label=r"$\varepsilon$ This work")
#    plt.loglog(freq_list, e_stein_list,'o', label=r"$\varepsilon_{Steinberg}$")
    plt.loglog(freq_arr, ef, label="Fit Krupar et al. 2020")
    plt.legend()
    plt.xlabel("Frequency (MHz)")
    plt.ylabel(r"$\varepsilon$")
    plt.yticks([1e-1,1e0])
    plt.savefig("eps_vs_freq_krup.png")
#   np.savez("eps_plot.npz", freq_list, e_krup_list, R_list, fit_pos_list, fit_err_list) 
    plt.show()
